...
try {
    ...
    $period_year = (int) date('Y', strtotime($start_date));
    $period_month = (int) date('m', strtotime($start_date));
    ...
    $stmt_providers = $conexion->prepare($sql_providers);
    $stmt_providers->execute();
    $providers = $stmt_providers->fetchAll();
    ...
    $sql_by_operation = "
        ...
        AND (
            e.rental_start_date IS NULL
            OR e.rental_start_date < :end_date
        )

        AND (
            e.rental_end_date IS NULL
            OR e.rental_end_date >= :start_date
        )
    ...
    $stmt_by_operation = $conexion->prepare($sql_by_operation);
    ...
    $billing_by_operation = $stmt_by_operation->fetchAll();
    ...
    $stmt_by_business = $conexion->prepare($sql_by_business);
    ...
    $billing_by_business = $stmt_by_business->fetchAll();
    ...
    /* RESUMEN SEGUN DETALLE FILTRADO */
    ...
    foreach ($billing_equipment_detail as $row) {

        $rental = (float) $row['rental_cost'];
        $hours_extra = (float) $row['hours_extra'];
        $repairs = (float) $row['repair_costs'];
        $other = (float) $row['other_costs'];
        $recoveries = (float) $row['recoveries'];
        $downtime = (float) $row['downtime_discount'];
        ...
        $summary['rental_cost'] += $rental;
        $summary['hours_extra'] += $hours_extra;
        $summary['repair_costs'] += $repairs;
        $summary['other_costs'] += $other;
        $summary['recoveries'] += $recoveries;
        $summary['downtime_discount'] += $downtime;
        $summary['total'] += $total;
    }
